...
class _PhotoPageState extends State<PhotoPage> {
  ...
  _buildAppBar(photo, bottomSheetList) => AppBar(
        ...
        actions: <Widget>[
          IconButton(
            onPressed: () =>
                Sheets.showBottomModalSheet(context, bottomSheetList),
            icon: Icon(
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: Text(
            photo.description ?? S.of(context).defaultDescription, maxLines: 3,
  ...
  void _getMyCollections(int page) async {
    if (!isLoadingCol) {
      ...
      var tempList = await DataProvider.getCollectionsByUser(
          globals.gMyProfile!.username!, page, 20);
              ...
              ListTile(
                ...
                onTap: () {
                  ...
                  condition
                      ? setState(() {
                          _addToCollection(
                              myColList[index].id!, widget.photo.id, false);
                          colIds.remove(myColList[index].id!);
                        })
                      : setState(() {
                          _addToCollection(
                              myColList[index].id!, widget.photo.id, true);
                          colIds.add(myColList[index].id!);
                        });
                },
              ),
          ...
          child: Material(
            ...
            child: Container(
              ...
              child: Container(
                ...
                child:
                    const CircularProgressIndicator(), // если что, добавить примари колор
              ),
            ),
          ),
        ),
